...
public class InteractionGlow : MonoBehaviour
{
    ...
    private Material[] _material=new Material[10];
    ...
    void Start()
    {
        ...
        for(int i = 0; i < 10; i++)
        {
            renderer = GetComponent<Transform>().GetChild(i).GetChild(1).GetComponent<Renderer>();

            if (renderer != null)
            {
                _material[i] = renderer.material;
            }
        }
    ...
    void Update()
    {
        for (int i=0; i<10;i++)
        {
            if (_material[i] != null)
            {
                ...
                _material[i].color = Color.Lerp(_material[i].color, targetColor, 30F * Time.deltaTime);
            }
        }
